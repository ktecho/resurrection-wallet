...
use nokhwa::{
    nokhwa_initialize, pixel_format::RgbAFormat, query, utils::ApiBackend
};
use tauri::Emitter;
...
async fn start_camera(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let mut camera = nokhwa::Camera::new(first_camera.index().clone(), format)
    ...
    while CAMERA_RUNNING.load(Ordering::SeqCst) {
        ...
        let image = frame.decode_image::<RgbAFormat>().map_err(|e| e.to_string())?;
        ...
        for result in results {
            if let Ok(content) = result {
                ...
                app.emit("qr-detected", content).map_err(|e| e.to_string())?;
                CAMERA_RUNNING.store(false, Ordering::SeqCst);
            }
        }
        ...
        println!("     ----- Emitting camera-frame !!!!!!!!!!");
        app.emit("camera-frame", base64).map_err(|e| e.to_string())?;
